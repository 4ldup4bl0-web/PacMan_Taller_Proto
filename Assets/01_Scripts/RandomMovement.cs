...
public class RandomMovement : MonoBehaviour
{
    ...
    void Start()
    {
        // Se inicializa con una direcci�n aleatoria que se chequear� en SieteAgallas
        ForceNewRandomDirection();
    }

    void Update()
    {
        // L�gica de temporizaci�n: solo si el script est� activo (es decir, en modo aleatorio)
        timer += Time.deltaTime;
        if (timer >= changeActionInterval)
        {
            // Forzar un cambio de direcci�n
            ForceNewRandomDirection();
        }
        ...
        if (moveDirection != Vector2.zero)
        {
            if (CheckForWall())
            {
                // Si detecta una pared, busca una direcci�n libre inmediatamente
                ForceNewRandomDirection();
                return;
            }
    ...
    /// <summary>
    /// Intenta forzar una direcci�n ortogonal que NO est� bloqueada por una pared.
    /// Este m�todo soluciona el problema de "cuesta moverse" al salir de la persecuci�n.
    /// </summary>
